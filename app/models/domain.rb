...
class Domain < ApplicationRecord
  ...
  def get_inside_mailbox
    request_url = "https://pddimp.yandex.ru/api/user_oauth_token.xml?domain=#{self.domain.domainname}&token=#{self.domain.domaintoken}&login=#{self..mailname}"
    request = RestClient::Request.execute(method: :get, url: request_url )
  ...
  def killfilter
    request = RestClient::Request.execute(method: :get, url: "https://pddimp.yandex.ru/delete_forward.xml?token=#{@domain.domaintoken}&login=#{params[:email]}&filter_id=#{params[:filter]}")
    result = Hash.from_xml(Nokogiri::Slop(request).to_s)
    if result.fetch("page").fetch("error", false)
      if result.fetch("page", "page").fetch("error", "error").fetch("reason", "reason") == "no_address"
